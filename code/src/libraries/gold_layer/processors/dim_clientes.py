...
class GoldDimClientesProcessor(BaseGoldProcessor):
    """Customer dimension (gold). One row per ID-group; keeps first NumeroCliente and representative name."""

    TABLE_COMMENT = "Customer dimension from silver.facturas. Stable NumeroCliente per ID-group (append-only)."
    DDL_COLUMNS_SQL = (
    ...
    def create_external_table(self, df_initial: DF) -> None:
        ...
        df_initial = df_initial.select("NumeroCliente", "Identificacion", "NombreCliente") \
    ...
    # ---------- MERGE (insert-only by representative name) ----------
    def merge_insert_new_names(self, df_new: DF) -> None:
        """Insert new representative names (one per group_key)."""
        logger.info("Running MERGE (insert-only by representative NombreCliente) into GOLD.dim_clientes")

        df_new = (
            df_new.withColumn(
                        "rn", F.row_number().over(
                            Window.partitionBy("group_key").orderBy(F.col("first_seen_ts").asc(), F.col("NumeroCliente").asc())
                        )
                    )
                  .where(F.col("rn") == 1)
                  .drop("rn")
    ...
    def process(self) -> None:
        ...
        tgt = spark.table(self.target_fullname).select("NombreCliente", "NumeroCliente")
        new_names = dim_current_all.join(tgt.select("NombreCliente"), on="NombreCliente", how="left_anti").distinct()
        ...
        to_insert = (
            new_names.withColumn("rn_tmp", F.row_number().over(w_new) + F.lit(max_n))
                     .select(
