...
class GoldDimProductosProcessor(BaseGoldProcessor):
    """Products dimension (gold). Built from silver.facturas + silver.detalle_facturas with stable surrogate id."""

    TABLE_COMMENT = "Products dimension (gold). Built from silver.facturas + silver.detalle_facturas with stable surrogate id."
    COLUMNS_COMMENT = {
        "IdProducto": "Surrogate product id (stable, incremental)",
        "NombreProducto": "Normalized product name"
    }

    # -------------------- Build source like the view --------------------
    ...
    def filter_base(self, norm: DF) -> DF:
        ...
        return (norm
                .where(~F.col("NombreProductoNorm").contains("OFERTA"))
    ...
    def map_name_fixes(self, base: DF) -> DF:
        ...
        mapped = (base.select(
            ...
            .otherwise(c).alias("NombreProductoNorm"),
            F.col("FechaCreacion")
        ))
    ...
    def compute_first_seen(self, mapped: DF) -> DF:
        ...
        return (mapped
                .groupBy("NombreProductoNorm")
                .agg(F.min(F.to_timestamp(F.col("FechaCreacion"))).alias("first_seen_ts")))

    def compute_initial_dim(self, first_seen: DF) -> DF:
        """Assign IdProducto by dense_rank over (first_seen_ts, NombreProductoNorm)."""
        w_dense = Window.orderBy(F.col("first_seen_ts").asc_nulls_last(),
                                 F.col("NombreProductoNorm").asc())
        return (first_seen
                .withColumn("IdProducto", F.dense_rank().over(w_dense))
                .select(
                    F.col("IdProducto").cast("int").alias("IdProducto"),
                    F.col("NombreProductoNorm").alias("NombreProducto")
                ))

    # -------------------- Table create (explicit DDL) --------------------
    ...
    # -------------------- Merge (insert-only by product name) --------------------
    ...
    def process(self) -> None:
        ...
        if not self.table_exists():
            ...
            tgt = spark.table(self.target_fullname).select("NombreProducto", "IdProducto")

            dim_current = self.compute_initial_dim(first_seen)  # recompute order signal
            new_names = (dim_current
                         .join(tgt.select("NombreProducto"), on="NombreProducto", how="left_anti")
                         .distinct())
