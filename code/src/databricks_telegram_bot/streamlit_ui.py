...
class StreamlitGenieChat:
    ...
    def display_sample_queries(self):
        ...
        sample_queries = [
            ...
            "Muéstrame el total de ventas por mes en el año 2022",
            ...
            "Calcula el promedio de descuentos aplicados por factura"
        ]
    ...
    def format_dataframe_for_display(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df_display.columns:
            if df_display[col].dtype == 'object' and col not in df.select_dtypes(include=['number']).columns:
    ...
    def format_response_for_streamlit(self, response: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if data:
                ...
                logger.info(f"Columns in result: {result.get('columns', 'No columns found')}")
                ...
                if columns and data:
                    if isinstance(data[0], list):
                        ...
                        logger.info("Processing as array format")
                        ...
                        for col in df.columns:
                            ...
                            if not numeric_series.isna().all():  # If any values converted successfully
                                df[col] = numeric_series.fillna(df[col])  # Fill failed conversions with original
                    ...
                    if isinstance(data[0], list):
                        ...
                        logger.info(f"Created generic columns: {columns}")
                        df = pd.DataFrame(data, columns=columns)
                ...
                if df is not None:
                    ...
                    if len(df) <= 100:
                        ...
                        st.warning(f"⚠️ Conjunto de resultados grande ({len(df)} filas). Mostrando las primeras 100 filas.")
                    ...
                    if len(numeric_columns) > 0:
                        ...
                        if len(numeric_columns) > 0:
                            ...
                            if len(df) <= 20:  # For small datasets, show all values
                                ...
                                chart_data = top_10.set_index(top_10.columns[0])[first_numeric]
                                st.bar_chart(chart_data)
                                st.caption("Mostrando los 10 valores más altos")
                    
                    # Optional: Show raw JSON data in collapsible section
                    with st.expander("📄 Ver datos originales en formato JSON", expanded=False):
                        st.json(data)
                
                else:
                    # Fallback: display as formatted JSON if DataFrame creation failed
                    logger.warning("Could not create DataFrame, falling back to JSON display")
                    st.warning("⚠️ No se pudo procesar los datos como tabla. Mostrando datos en formato JSON.")
                    
                    with st.expander("📄 Ver datos en formato JSON (opcional)", expanded=False):
                        st.json(data)
